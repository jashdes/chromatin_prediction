function res = bpass_plusEdges(image_array,lnoise,lobject,threshold)
% based on Crocker and Grier's IDL code 
% NAME:
%               bpass
% PURPOSE:
...
%               Image Processing
% CALLING SEQUENCE:
%               res = bpass( image_array, lnoise, lobject, threshold )
% INPUTS:
%               image:  The two-dimensional array to be filtered.
%               lnoise: Characteristic lengthscale(pixels) of noise in pixels.
...
% 
% % Zero out the values on the 4 edges.  Why?(gh).     
% lzero = int8(max(lobject/5,ceil(5*lnoise))); %int8, divide by 5 added by gh 12/2015
%     % lzero = max(lobject,ceil(5*lnoise)); % changed by gh 12/2015
% 
